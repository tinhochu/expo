...
open class FileDownloader(context: Context) {
  ...
  private fun parseManifestResponse(response: Response, configuration: UpdatesConfiguration, callback: ManifestDownloadCallback) {
    ...
    if (match.find()) {
      parseMultipartManifestResponse(response, match.group(1), configuration, callback)
    } else {
  ...
  private fun parseMultipartManifestResponse(response: Response, boundary: String, configuration: UpdatesConfiguration, callback: ManifestDownloadCallback) {
    ...
    var assetHeadersBody: String? = null
      ...
      object : MultipartStreamReader.ChunkListener {
        ...
        override fun onChunkComplete(
          ...
          when (currChunkIndex) {
            ...
            1 -> assetHeadersBody = body.readUtf8()
          }
    ...
    parseManifest(manifestBody!!, Headers.of(manifestHeaders!!), assetHeadersBody, configuration, callback)
  }

  private fun parseManifest(manifestBody: String, manifestHeaders: Headers, assetHeadersBody: String?, configuration: UpdatesConfiguration, callback: ManifestDownloadCallback) {
    try {
      ...
      val extraAssetHeaders = assetHeadersBody?.let { JSONObject(it) }
      ...
      if (signature != null && !isUnsignedFromXDL) {
        Crypto.verifyPublicRSASignature(
          ...
          object : RSASignatureListener {
            ...
            override fun onCompleted(isValid: Boolean) {
              if (isValid) {
                try {
                  createManifest(preManifest, manifestHeaders, extraAssetHeaders,true, configuration, callback)
                } catch (e: Exception) {
        ...
        createManifest(preManifest, manifestHeaders, extraAssetHeaders,false, configuration, callback)
      }
    } catch (e: Exception) {
      callback.onFailure(
  ...
  companion object {
    ...
    private fun createManifest(
      ...
      val updateManifest = ManifestFactory.getManifest(preManifest, headers, extraAssetHeaders, configuration)
